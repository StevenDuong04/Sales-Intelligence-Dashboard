...
if "dataset" in st.session_state and st.session_state["dataset"] is not None:
    ...
    with col1:
        st.metric(label="Total Transactions", value=f"{kpis.calculate_total_orders(df):,}")
    with col2:
    ...
    with col4:
        st.metric(label="Average Order Value", value=f"${kpis.calculate_average_order_value(df):,.2f}")

    st.subheader("Revenue Trend")
    revenue_growth_trend = trends.monthly_revenue_trend(df)
    st.plotly_chart(charts.create_line_chart(data=revenue_growth_trend, x='month', y='total_revenue', title='Revenue Growth Trend'))

    product_col, customer_col = st.columns(2)
    with product_col:
        st.subheader("Top Products")
        top_products = products.top_performing_product(df)
